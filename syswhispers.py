...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="SysWhispers3 - SysWhispers on steroids"
    )
    parser.add_argument(
        "-p",
        "--preset",
    ...
    parser.add_argument(
        ...
        "--compiler",
        default="msvc",
        choices=["msvc", "mingw", "all"],
        help="Compiler (default: msvc)",
        required=False,
    )
    parser.add_argument(
        "-m",
        ...
        help="Syscall recovery method (default: embedded)",
        required=False,
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "-o",
        "--out-file",
        help="Output basename (w/o extension)",
        default="syscalls",
        required=False,
    )
    parser.add_argument(
        ...
        default=False,
        action="store_true",
        help="Use the old `int 2eh` instruction in place of `syscall`",
        required=False,
    )
    parser.add_argument(
        "--wow64",
        default=False,
        action="store_true",
        help="Add support for WoW64, to run x86 on x64",
        required=False,
    )
    parser.add_argument(
        "-s",
        ...
        default=False,
        action="store_true",
        help="Generate a single header file",
        required=False,
    )
    ...
    shout_level.add_argument(
        ...
        default=False,
        action="store_true",
        help="Enable debug output and syscall debug (insert software breakpoint)",
        required=False,
    )
    ...
    try:
        if args.preset == "all":
            ...
            logger.error(
                "ERROR:   --preset XOR --functions switch must be specified.\n"
            )
            logger.error(
                "EXAMPLE: ./syswhispers.py --preset common --out-file syscalls_common"
            )
            logger.error(
                "EXAMPLE: ./syswhispers.py --functions NtTestAlert,NtGetCurrentProcessorNumber --out-file syscalls_test"
            )
